...
const MeetingsWidget : React.FC = () => {
    ...
    const rows = meetings.map((row) => (
        <Table.Tr key={row.meetingId}>
            <Table.Td>{row.name}</Table.Td>
            <Table.Td>{dayjs(adjustTimeToLocal(row.date)).format('DD.MM.YYYY')}</Table.Td>
            <Table.Td>{dayjs(adjustTimeToLocal(row.date)).format('HH:mm')}</Table.Td>
                            ...
                            <Table miw={700} stickyHeader>
                                <Table.Thead  className={clsx(classes.header, { [classes.scrolled]: scrolled })}>
                                    <Table.Tr>
                                        ...
                                        <Table.Th>Godzina rozpoczęcia</Table.Th>
